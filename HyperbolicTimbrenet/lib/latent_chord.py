...
class latent_chord:
    ...
    def plt_color(self):
        return self.colors[self.triads.index(self.triad)][
            self.octaves.index(self.octave)
        ]
    ...
    def one_hot_label(self):
        ...
        one_hot[
            0, self.volumes.index(self.volume) + len(self.triads) + len(self.octaves)
        ] = 1
        one_hot[
            0,
            self.instruments.index(self.instrument)
            + len(self.triads)
            + len(self.octaves)
            + len(self.volumes),
        ] = 1
    ...
    def from_file(cls, path, filename, model, spec_helper, hyp=False):
        ...
        (
            ...
            chord_octave,
            new_latent_chord.volume,
            new_latent_chord.example,
        ) = filename.split("_")
