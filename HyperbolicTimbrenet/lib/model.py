...
class CVAE(tf.keras.Model):
    ...
    def encode(self, x):
        # print(self.inference_net(x))
        # a = self.inference_net(x)
        # # reshape a
        # a = tf.reshape(a, [5,2,2])
        # b = a
        # print(a)
        # concat = tf.concat([a,b], axis=2)
        # print(concat)
    ...
    def __init__(self, latent_dim):
        ...
        self.inference_net = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Flatten(),
                LinearHyperbolicPlusPlus(4, Poincare(), 1),
            ]
        )
    ...
    def __init__(self, latent_dim):
        ...
        self.inference_net = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Dropout(0.25),
                LinearHyperbolic(
                    latent_dim + latent_dim, Poincare(), 1, input_hyp=True
                ),
            ]
        )
    ...
    def __init__(self, latent_dim):
        ...
        self.embedding = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.permute((1, 3, 2)),
                tf.keras.layers.Reshape((128, 16, 16)),
