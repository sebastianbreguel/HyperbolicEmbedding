...
def timbrenet_generate_latent_map(
    ...
    if latent_dim == 2:
        ...
        Y = np.zeros(
            (len(latent_dataset), np.shape(latent_dataset[0].one_hot_label)[1])
        )
    ...
    legend_mkr_list = list(
        itertools.product(
            mkr_type, list(itertools.chain.from_iterable(colors)), mkr_size
        )
    )
    ...
    if latent_dim == 2:
        ...
        for j in range(len(latent_dataset)):
            element = ax.scatter(
                X_cca[j, 0],
                X_cca[j, 1],
                c=latent_dataset[j].plt_color,
                marker=latent_dataset[j].plt_mkr_type,
                s=latent_dataset[j].plt_mkr_size,
            )
            legen_data = (
                latent_dataset[j].plt_mkr_type,
                latent_dataset[j].plt_color,
                latent_dataset[j].plt_mkr_size,
            )
            if legen_data in legend_mkr_list:
                i = legend_mkr_list.index(legen_data)
                name = legend_name_list[i]
